...
def _git_short(root: Path) -> str | None:
    try:
        r = subprocess.run(
            ["git", "rev-parse", "--short", "HEAD"],  # noqa: S607 — git on PATH is the universal invariant
            cwd=root,
            capture_output=True,
            text=True,
            timeout=5,
...
def _git_branch(root: Path) -> str | None:
    try:
        r = subprocess.run(
            ["git", "rev-parse", "--abbrev-ref", "HEAD"],  # noqa: S607 — git on PATH is the universal invariant
            cwd=root,
            capture_output=True,
            text=True,
            timeout=5,
