"""
Structured logging configuration for Inkscape MCP Server using loguru.
...
class StructuredLogger:
    """
    Structured logging manager for Inkscape MCP Server.
...
def log_operation_success(context: dict[str, Any], result: Any = None) -> None:
    ...
    logger.info(
        ...
        extra={"context": {**context, "end_time": end_time, "result": result, "status": "success"}},
    )


def log_operation_error(context: dict[str, Any], error: Exception, error_details: dict[str, Any] | None = None) -> None:
    """
    ...
    logger.error(
        f"Operation failed: {context['operation']} - {error!s}",
        extra={
            "context": {
                **context,
                "end_time": end_time,
...
def log_tool_result(context: dict[str, Any], result: Any, success: bool = True) -> None:
    ...
    log_func(
        ...
        extra={"tool_result": {**context, "end_time": end_time, "result": result, "status": status}},
    )
...
def log_server_start(port: int | None = None, mode: str = "stdio") -> None:
    """Log server startup."""
    logger.info(
        f"Inkscape MCP Server starting in {mode} mode",
...
def log_server_stop(reason: str = "shutdown") -> None:
    """Log server shutdown."""
    logger.info(f"Inkscape MCP Server stopping: {reason}", extra={"server_stop": {"reason": reason}})
...
def log_tool_registration(category: str, tool_count: int, success: bool = True) -> None:
    ...
    logger.info(
        ...
        extra={"tool_registration": {"category": category, "tool_count": tool_count, "status": status}},
    )
...
def log_inkscape_detection(inkscape_path: str | None, success: bool) -> None:
    ...
    if success and inkscape_path:
        logger.info(
            f"Inkscape detected at: {inkscape_path}",
            extra={"inkscape_detection": {"path": inkscape_path, "found": True}},
        )
    else:
        logger.warning(
            "Inkscape not found - running in limited mode",
            extra={"inkscape_detection": {"found": False}},
        )
